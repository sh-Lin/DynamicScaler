...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps, context=None, features_adapter=None, fps=16, timestep_cond=None, **kwargs):
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        if timestep_cond is not None:
            t_emb = t_emb.to(self.dtype) + self.time_cond_proj(timestep_cond).to(self.dtype)
        emb = self.time_embed(t_emb)

        if self.fps_cond:
            if type(fps) == int:
                fps = torch.full_like(timesteps, fps)
            fps_emb = timestep_embedding(fps,self.model_channels, repeat_only=False).to(self.dtype)
