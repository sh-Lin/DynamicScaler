...
import torch
from lvdm.models.utils_diffusion import make_ddim_sampling_parameters, make_ddim_timesteps, make_ddim_timesteps_lcm
...
class DDIMSampler(object):
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True, use_lcm=False):

        if use_lcm:
            self.ddim_timesteps = make_ddim_timesteps_lcm(num_inference_steps=ddim_num_steps,
                                                          num_train_timesteps=self.ddpm_num_timesteps,
                                                          verbose=verbose)
        else:
    ...
    def sample(self,
        ...
        self.make_schedule(ddim_num_steps=S, ddim_eta=eta, verbose=schedule_verbose, use_lcm=use_lcm)
    ...
    def ddim_sampling(self, cond, shape,
                      ...
                      cond_tau=1., target_size=None, start_timesteps=None, tmp_dir=None,
                      **kwargs):
        ...
        for i, step in enumerate(iterator):
            if i == 0 and tmp_dir is not None:
                torch.save(img, f"{tmp_dir}/{i}.pt")
        ...
        if tmp_dir is not None:
            torch.save(img, f"{tmp_dir}/{total_steps}.pt")
