...
class T2VTurboScheduler(SchedulerMixin, ConfigMixin):
    ...
    def fifo_step(
            ...
            timeindex_list,
            timestep_list,
            sample_latent: torch.FloatTensor,
            return_dict: bool = True,
    ):
        if self.num_inference_steps is None:
            raise ValueError(
        ...
        for i, time_index in enumerate(timeindex_list):

            model_output = model_pred[:, :, [i]]
            sample = sample_latent[:, :, [i]]

            timestep = timestep_list[i] # doubt 这里 timestep 不应该就是 self.timesteps[time_index] 吗?

            # 1. get previous step value
            prev_timeindex = time_index - 1 # doubt: 此处逆序 timesteps + 1 变成 - 1 ??
            if 0 <= prev_timeindex < len(self.timesteps):
                prev_timestep = self.timesteps[prev_timeindex]  # Notes: 这里 timesteps 已经被 fifo_sample 修改了
            ...
            prev_samples_list.append(prev_sample)
            denoiseds_list.append(denoised)

        prev_sample = torch.cat(prev_samples_list, dim=2)
        denoised = torch.cat(denoiseds_list, dim=2)
